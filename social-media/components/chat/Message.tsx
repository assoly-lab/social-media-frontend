        ...
        {userProfile && userProfile?.user?.id == message.sender.user.id ?
            <div className="message flex flex-col gap-3 w-full text-black  pr-4 items-start overflow-hidden my-2">
                <div className="flex flex-row-reverse self-end gap-2">
                    <Image src={message.sender.avatar.startsWith('http') ? message.sender.avatar : `https://tornado008.pythonanywhere.com${message.sender.avatar}`} width={25} height={25} alt="user's profile avatar" className="object-contain rounded-full mb-4" />
