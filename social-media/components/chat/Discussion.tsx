...
export default function Discussion({ user }: { user: any }) {
  ...
  const [lastMessageHeight, setLastMessageHeight] = useState<
    number | undefined
  >(0);
    ...
    if (oldMessages) {
      try {
        ...
        setMessages((prev: MessageType) => {
          return { ...prev, [user.id]: [...data.results, ...prev[user.id]] };
        });
      ...
      if (user) {
        try {
          const response = await fetchWithAuth(
            `https://tornado008.pythonanywhere.com/api/discussions/${user.id}/`
          );
          if (!response.ok) {
            const error = await response.json();
            throw new Error(error);
          ...
          const data = await response.json();
          setMessages((prev: MessageType) => {
            return { ...prev, [user.id]: [...data.results] };
          });
          setOldMessages(data.next);
          if (messagesDivRef.current) {
            setLastMessageHeight(messagesDivRef.current.scrollHeight);
  ...
  useEffect(() => {
    if (!isLoading) {
      if (messagesDivRef.current) {
        if (lastMessageHeight)
          messagesDivRef.current.scrollTop =
            messagesDivRef.current.scrollHeight - lastMessageHeight;
      }
    }
  }, [isLoading]);
  ...
  return (
    <div className="w-full flex flex-col gap-2">
        ...
        <Image
          ...
          src={
            user.avatar.startsWith("http")
              ? user.avatar
              : `https://tornado008.pythonanywhere.com/media/${user.avatar}`
          }
          width={50}
          height={50}
      ...
      <div
        ref={messagesDivRef}
        className="bg-white w-full h-full overflow-auto gap-2 py-2"
      >
        {isLoading && (
        ...
        {messages[user.id] &&
          ...
          messages[user.id].map((message: MessagePayload) => {
            return <Message key={message.id} message={message} />;
          })}
      </div>
      <form
        className="w-[90%] flex gap-4 mx-auto "
        action={async (formData: FormData) => sendMessage(formData)}
      >
        <textarea
          ref={messageInputRef}
          name="message"
          ...
          className="focus:outline-none focus:border focus:border-[#7F265B] flex-1 p-2 rounded-md bg-slate-100"
        />
        <input
          type="submit"
          value="Send"
          className="text-white bg-[#7F265B] py-2 px-3 h-fit rounded-md self-end"
        />
      </form>
    </div>
  );
