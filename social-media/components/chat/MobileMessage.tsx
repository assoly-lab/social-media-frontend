    ...
    return (
        ...
        {userProfile && userProfile?.user?.id == message.sender.user.id ?
            <div className="message flex flex-row-reverse gap-3 w-full text-black self-end pr-4 items-start overflow-hidden my-2">
                <Image src={message.sender.avatar.startsWith('http') ? message.sender.avatar : `http://localhost:8000${message.sender.avatar}`} width={25} height={25} alt="user's profile avatar" className="object-contain" />
            ...
            <div className="message flex gap-3 w-full text-white items-start overflow-hidden my-2">
                <Image src={message.sender.avatar.startsWith('http') ? message.sender.avatar : `http://localhost:8000${message.sender.avatar}`} width={25} height={25} alt="user's profile avatar" className="object-contain rounded-full" />
